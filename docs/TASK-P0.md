...
sw.js           APP_SHELL に core/*.js を追加、CACHE_NAME を v8 へ
...
2. **`sw.js`** — 新しいモジュールがキャッシュされないと、オフラインで真っ白になります。
   ```js
   const CACHE_NAME = "fingerstyle-practice-v8";   // v7 から上げる
   ```
...
- DevTools の Application → Service Workers に `v8` のキャッシュが登録され、`core/*.js` が入っていること
...
- [ ] `sw.js` の `CACHE_NAME` が `v8`、`APP_SHELL` に `core/*.js` の3ファイルがある
